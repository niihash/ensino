...
<section id="content" class="flex-1 p-6">
    <div class="bg-white rounded-xl shadow p-6 min-h-[calc(100vh-3rem)]">
        <form action="/matriculas/{<?= $matricula->id ?>}/edit" method="get" class="space-y-6 max-w-xl">
            ...
            <div>
                ...
                <select name="aluno_id" id="aluno_id" required disabled>
                    <?php foreach ($listAluno as $aluno): ?>
                        <option value="<?= $aluno->id ?>"
                        <?= $matricula->aluno_id === $aluno->id ? "selected" : "" ?> ><?= $aluno->nome ?></option>
                    <?php endforeach ?>
                </select>
            </div>
