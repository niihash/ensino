...
<section id="content" class="flex-1 p-6">
    <div class="bg-white rounded-xl shadow p-6 min-h-[calc(100vh-3rem)]">
        ...
        <form action="/alunos" method="get" class="mb-4 flex gap-2">
            <input type="text" name="q" placeholder="Buscar por nome ou e-mail"
                value="<?= isset($_GET['q']) ? htmlspecialchars($_GET['q']) : '' ?>"
                class="px-4 py-2 border rounded-lg flex-1 focus:outline-none focus:ring-2 focus:ring-primary/50">
            <button type="submit"
                class="px-4 py-2 bg-primary text-white font-medium rounded-lg hover:bg-red-600 transition">
                Buscar
            </button>
        </form>
